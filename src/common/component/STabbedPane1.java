...
public class STabbedPane1 extends JTabbedPane {
	 ...
	 * 
	 * @param arg0
	 *            该参数为true时，无论是否选中，都显示可关闭按钮， 为false时，只有选中时才显示
	 */
	public STabbedPane1(boolean arg0) {
		super();
		showClose = arg0;
		initialize();
	}
	 ...
	 *            是否可关闭
	 */
	public void addTab(String title, String iconPath, Component component,
	...
	class STabbedPaneUI extends BasicTabbedPaneUI {
		...
		protected void paintTabBackground(Graphics g, int tabPlacement,
			...
			case LEFT:
				if (isSelected) {
					gradient = new GradientPaint(x + 1, y, colorNorth, x + w,
							y, colorSouth, true);
				} else {
					gradient = new GradientPaint(x + 1, y, Color.LIGHT_GRAY, x
							+ w, y, Color.WHITE, true);
				}
			...
			case RIGHT:
				if (isSelected) {
					gradient = new GradientPaint(x + w, y, colorNorth, x + 1,
							y, colorSouth, true);
				} else {
			...
			case BOTTOM:
				if (isSelected) {
					...
					gradient = new GradientPaint(x + 1, y + h,
							Color.LIGHT_GRAY, x + 1, y, Color.WHITE, true);
				}
				g2d.setPaint(gradient);
			...
			default:
				if (isSelected) {
					gradient = new GradientPaint(x + 1, y, colorNorth, x + 1, y
							+ h, colorSouth, true);
				} else {
			...
			protected void calculateTabRects(int tabPlacement, int tabCount) {
				...
				case LEFT:
					...
					returnAt = size.height
							- (insets.bottom + tabAreaInsets.bottom);
					break;
				case RIGHT:
					maxTabWidth = calculateMaxTabWidth(tabPlacement) + 20;
				...
				if (runCount > 1) {
					normalizeTabRuns(tabPlacement, tabCount,
							verticalTabRuns ? y : x, returnAt);
